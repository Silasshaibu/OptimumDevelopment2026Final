...
$stmt = $pdo->prepare("SELECT * FROM products WHERE category = 'handheld' AND is_featured = 1 ORDER BY display_order ASC, created_at DESC");
$stmt->execute();
$carouselSlides = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Fetch regular products
$stmt = $pdo->prepare("SELECT * FROM products WHERE category = 'handheld' AND is_featured = 0 ORDER BY display_order ASC, created_at DESC");
$stmt->execute();
...
<div class="admin-card">
    <div class="admin-card-header">
        ...
        <div class="admin-actions">
            <a href="product_add.php?category=handheld&type=carousel" class="btn btn-primary">
    ...
    <p class="section-description" style="padding: 0 20px; color: #666; margin-bottom: 15px;">
        These are the hero banner slides that appear in the carousel at the top of the Handheld Devices section.
    </p>
    
    <div class="admin-card-body">
        <?php if (empty($carouselSlides)): ?>
            <div class="empty-state">
                <p>No carousel slides found. <a href="product_add.php?category=handheld&type=carousel">Add your first slide</a></p>
            </div>
        <?php else: ?>
...
<div class="admin-card" style="margin-top: 30px;">
    <div class="admin-card-header">
        <h2>📦 Products Grid</h2>
        <div class="admin-actions">
            <a href="product_add.php?category=handheld&type=product" class="btn btn-primary">
    ...
    <div class="admin-card-body">
        <?php if (empty($products)): ?>
            <div class="empty-state">
                <p>No products found. <a href="product_add.php?category=handheld&type=product">Add your first product</a></p>
            </div>
        <?php else: ?>
            <div class="table-responsive">
                <table class="admin-table">
